...
export default function MinhaRedePage() {
  ...
  // Lista dinâmica de cidades e bairros únicos para os selects de filtro
  const citiesList = useMemo(() => {
    if (!data?.items) return [];
    const set = new Set<string>();
    data.items.forEach((item) => { if (item.city) set.add(item.city); });
  ...
  const neighborhoodsList = useMemo(() => {
    if (!data?.items) return [];
    const set = new Set<string>();
    data.items.forEach((item) => { if (item.neighborhood) set.add(item.neighborhood); });
    return Array.from(set).sort();
  }, [data]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return (data?.items || []).filter((item) => {
      ...
      const matchesNeighborhood = neighborhoodFilter === 'todos' || item.neighborhood === neighborhoodFilter;
      ...
      return matchesLevel && matchesStatus && matchesCity && matchesNeighborhood && matchesSearch;
    });
  }, [data, level, statusFilter, cityFilter, neighborhoodFilter, search]);
      ...
      <div className={styles.page}>
        ...
        <section className={styles.contentCard}>
          <div className={styles.toolbar}>
            {/* Linha superior da toolbar: Busca + Filtros por Nível */}
            <div className={styles.topRow}>
              <label className={styles.search}>
                ...
                <input
                  ...
                  placeholder="Buscar por nome, usuário, cidade ou bairro"
                  aria-label="Buscar na minha rede"
                />
              </label>
              ...
              <div className={styles.filters} aria-label="Filtrar por nível">
                {(['todos', '1', '2', '3'] as LevelFilter[]).map((item) => (
                  <button
                    type="button"
                    key={item}
                    className={level === item ? styles.filterActive : ''}
                    onClick={() => setLevel(item)}
                  >
                    {item === 'todos' ? 'Todos os Níveis' : `Nível ${item}`}
                  </button>
                ))}
              </div>
            </div>

            {/* Linha inferior de Filtros: Status, Cidade, Bairro */}
          ...
          {loading ? (
            <div className={styles.state}>
              ...
              <strong>Carregando sua rede...</strong>
            </div>
          ) : error ? (
            ...
            <div className={styles.state}>
              ...
              <p>{data?.items.length ? 'Tente ajustar os filtros.' : 'Novos embaixadores aparecerão aqui conforme sua rede crescer.'}</p>
            </div>
          ) : (
              ...
              <table>
                ...
                <tbody>
                  {filteredItems.map((item) => (
                    <tr key={item.id}>
                      {/* Avatar + Nome (Foto do Perfil se houver) */}
                      <td>
                        <div className={styles.person}>
                          {item.photo_path ? (
                            <img
                              src={item.photo_path}
                              alt={item.display_name || item.full_name}
                              className={styles.avatarPhoto}
                            />
                          ) : (
                            <span className={styles.avatarSpan}>
                              {initials(item.display_name || item.full_name)}
                            </span>
                          )}
                          <div>
                      ...
                      <td>
                        <span className={`${styles.levelPill} ${styles[`level${item.level}`]}`}>
                          Nível {item.level}
                        </span>
                      </td>
                      ...
                      {/* Regra de Privacidade: WhatsApp APENAS no Nível 1 */}
                      <td>
                        {item.level === 1 && item.phone ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span style={{ fontSize: '13px', color: 'var(--color-on-surface)', whiteSpace: 'nowrap' }}>
                              {item.phone}
                            </span>
                            <a
                          ...
                          <span className={styles.privatePhoneBadge}>
                      ...
                      <td>
                        {[item.neighborhood, item.city, item.state].filter(Boolean).join(' · ') || 'Não informado'}
                      </td>
          ...
          {!loading && !error && data && data.items.length > 0 && (
            <footer className={styles.footer}>
              Exibindo <strong>{filteredItems.length}</strong> de <strong>{data.items.length}</strong> embaixadores da sua rede.
            </footer>
          )}
        </section>
      </div>

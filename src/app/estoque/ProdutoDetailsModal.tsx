...
import { Produto, EstoqueMovimentacao, Pedido } from '@/models/types';
import { getHistoricoProdutoAction, getPedidosDoProdutoAction, getPedidoByIdOrNumberAction } from './actions';
import { statusConfig } from '../vendas/pedidos/constants/workflow';
import PedidoDetailsModal from '../vendas/pedidos/components/PedidoDetailsModal';
...
export const ProdutoDetailsModal = ({ produto, onClose }: ProdutoDetailsModalProps) => {
  ...
  const handleOpenPedido = async (referenciaId?: string | null, observacoes?: string | null) => {
    let target: string | null = referenciaId || null;
    if (!target && observacoes) {
      const match = observacoes.match(/(PV\d+)/i);
      if (match) target = match[1];
    }
    if (!target) return;

    setLoadingPedidoId(target);
    try {
      const data = await getPedidoByIdOrNumberAction(target);
      if (data) {
        setSelectedPedido(data as Pedido);
      } else {
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {pedidos.length === 0 ? (
                  ...
                  pedidos.map(ped => {
                    ...
                    const targetId = ped.pedido_id || ped.numero_pedido;

                    return (
                      <div
                        key={ped.id}
                        onClick={() => handleOpenPedido(targetId)}
                        title="Clique para abrir os detalhes do pedido"
                        style={{
                          padding: '16px 20px',
                      ...
                      >
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          ...
                          <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--color-primary)', display: 'inline-flex', alignItems: 'center', gap: '3px', marginTop: '4px' }}>
                            <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>open_in_new</span>
                            {loadingPedidoId === targetId ? 'Carregando...' : 'Ver pedido'}
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {historico.length === 0 ? (
                  ...
                  const obsMatch = mov.observacoes ? mov.observacoes.match(/(PV\d+)/i) : null;
                  const orderTarget = mov.referencia_id || (obsMatch ? obsMatch[1] : null);
                  const isClickable = !!orderTarget;

                  return (
                    <div
                      key={mov.id}
                      onClick={() => isClickable && handleOpenPedido(orderTarget, mov.observacoes)}
                      title={isClickable ? 'Clique para abrir os detalhes do pedido' : undefined}
                      style={{
                    ...
                    >
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: '11px', color: 'var(--color-on-surface-variant)', maxWidth: '240px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '2px' }}>
                          {mov.observacoes || '-'}
                        </div>
                        {isClickable && (
                          <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--color-primary)', display: 'inline-flex', alignItems: 'center', gap: '3px', marginTop: '4px' }}>
                            <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>open_in_new</span>
                            {loadingPedidoId === orderTarget ? 'Carregando...' : 'Ver pedido'}
                          </div>
                        )}
                      </div>
                    </div>
